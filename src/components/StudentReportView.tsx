...
import { Profile, Session, AttendanceRecord, Assignment, AssignmentSubmission, SessionSummary, AbsenceRequest } from '../types';
import { exportToCSV as utilExportToCSV, exportToExcel as utilExportToExcel, formatReportDate, formatReportTime, formatReportDateTime } from '../utils/export';
import { getStudentAssignmentStatus } from '../utils/assignmentUtils';
...
export default function StudentReportView({
  ...
  const excusedSessionsList = useMemo(() => {
    return (absenceRequests || []).filter(r => 
      r.studentId === profile.id && 
      r.status === 'Approved' && 
      !studentAttendance.some(a => a.sessionId === r.sessionId)
    );
  }, [absenceRequests, profile.id, studentAttendance]);

  const excusedCount = excusedSessionsList.length;
  const attendedSessions = studentAttendance.length;
  const missedSessions = Math.max(0, totalSessions - attendedSessions - excusedCount);
  
  const denominator = Math.max(1, totalSessions - excusedCount);
  const attendancePercentage = totalSessions > 0 
    ? Math.round((attendedSessions / denominator) * 100) 
    : 0;
  ...
  const submittedCount = useMemo(() => {
    return assignments.filter(assign => {
      ...
      const status = getStudentAssignmentStatus(assign.deadline, sub?.submittedAt);
  ...
  // Dynamic feedback status badge derived from current session feedback deadlines
  const overallFeedbackStatus = useMemo(() => {
    if (!sessions || sessions.length === 0) {
      return {
        text: 'No Sessions',
        iconColor: 'text-slate-400',
        badgeBg: 'bg-slate-900/60 border-slate-800 text-slate-400'
      };
    }

    const checkedInSessionIds = new Set(studentAttendance.map(a => a.sessionId));
    const relevantSessions = sessions.filter(s => checkedInSessionIds.has(s.id) || getFeedbackWindowStatus(s).endDateTime <= new Date());
    const evalSessions = relevantSessions.length > 0 ? relevantSessions : sessions;
    ...
    for (const sess of evalSessions) {
      const ws = getFeedbackWindowStatus(sess);
      if (!ws.isExpired && !ws.isLocked) {
        hasOpen = true;
        allExpired = false;
        if (ws.isReopened) {
          hasReopened = true;
        }
      } else if (!ws.isExpired) {
        allExpired = false;
      }
    }
    ...
    if (allExpired && evalSessions.length > 0) {
      return {
        text: '🔴 Feedback Closed',
    ...
    return {
      text: '🔒 Feedback Locked',
      iconColor: 'text-amber-400',
      badgeBg: 'bg-amber-500/10 border-amber-500/20 text-amber-400'
    };
  }, [sessions, studentAttendance]);

  // Comparative Class Metrics Calculations (Real contextual stats)
  const classAverageAttendance = useMemo(() => {
    if (sessions.length === 0 || attendance.length === 0) return null;
    const uniqueStudentsInAttendance = new Set(attendance.map(a => a.studentId));
    const studentCount = Math.max(1, uniqueStudentsInAttendance.size);
    const averagePresentPerStudent = attendance.length / studentCount;
    const averagePercent = Math.round((averagePresentPerStudent / sessions.length) * 100);
    return Math.min(100, averagePercent);
  }, [sessions, attendance]);

  const classAverageSubmissionRate = useMemo(() => {
    if (assignments.length === 0 || submissions.length === 0) return null;
    const uniqueStudentsInSubmissions = new Set(submissions.map(s => s.studentId));
    const studentCount = Math.max(1, uniqueStudentsInSubmissions.size);
    const averageSubmissionsPerStudent = submissions.length / studentCount;
    const averagePercent = Math.round((averageSubmissionsPerStudent / assignments.length) * 100);
    return Math.min(100, averagePercent);
  ...
  const attendanceHistory = useMemo(() => {
    return sessions.map(session => {
      ...
      let status: 'Present' | 'Absent' | 'Excused Absence' = 'Absent';
      if (record) {
        status = 'Present';
      } else if (absenceRequests) {
        const req = absenceRequests.find(r => r.sessionId === session.id && r.studentId === profile.id);
      ...
      return {
        ...
        venue: session.venue,
        status,
        checkInTime: record ? formatReportTime(record.checkInTime) : 'N/A',
        method: record ? record.method : 'N/A'
      };
    });
  }, [sessions, studentAttendance, absenceRequests, profile.id]);
  ...
  const assignmentHistory = useMemo(() => {
    return assignments.map(assign => {
      ...
      return {
        id: assign.id,
        name: assign.title,
        deadline: formatReportDateTime(assign.deadline),
        submittedAt: sub ? formatReportDateTime(sub.submittedAt) : 'N/A',
        status
      };
    });
  }, [assignments, studentSubmissions]);
  ...
  const exportToCSV = () => {
    const filename = `student_report_${profile.usn || profile.fullName.replace(/\s+/g, '_')}`;
    const reportName = "Student Performance Report";
    const userName = profile.fullName;

    const headers = ['Category / Item', 'Detail 1', 'Detail 2', 'Detail 3', 'Detail 4', 'Detail 5', 'Detail 6'];
    const rows = [
      ['Student Information'],
      ...
      ['Assignments Closed', String(closedCount)],
      ['Assignments Late', String(lateCount)],
      [],
      ['Attendance History'],
      ['Session Name', 'Session Date', 'Venue', 'Status', 'Check-In Time', 'Method'],
      ...(attendanceHistory.length === 0 ? [['No Records Available']] : attendanceHistory.map(h => [
        h.name,
        formatReportDate(h.date),
        h.venue === 'N/A' || !h.venue ? 'Not Available' : h.venue,
        h.status,
        h.checkInTime === 'N/A' ? 'Not Available' : formatReportDateTime(h.checkInTime),
        h.method === 'qr' ? 'QR Code' : h.method === 'manual' ? 'Manual' : 'Not Available'
      ])),
      [],
      ['Assignment Status'],
      ['Assignment Title', 'Due Date', 'Submitted At', 'Completion Status'],
      ...(assignmentHistory.length === 0 ? [['No Records Available']] : assignmentHistory.map(a => [
        a.name,
        formatReportDate(a.deadline),
        a.submittedAt === 'N/A' ? 'Not Submitted' : formatReportDateTime(a.submittedAt),
        a.status
      ]))
    ];

    utilExportToCSV(filename, headers, rows, reportName, userName);
  };
  ...
  const exportToExcel = () => {
    ...
    const sheets = [
        ...
        sections: [
          {
            ...
            headers: ['Session Name', 'Session Date', 'Venue', 'Status', 'Check-In Time', 'Method'],
            rows: attendanceHistory.map(h => [
              h.name,
              formatReportDate(h.date),
              h.venue === 'N/A' || !h.venue ? 'Not Available' : h.venue,
              h.status,
      ...
      },
      {
        ...
        sections: [
          {
            title: 'Assignment Submissions & Status',
            headers: ['Assignment Title', 'Due Date', 'Submitted At', 'Completion Status'],
    ...
    utilExportToExcel(filename, reportName, userName, sheets);
  };
        ...
        <div className="md:col-span-8 grid sm:grid-cols-3 gap-4">
          ...
          <div className="glass-panel p-5 rounded-2xl flex flex-col justify-between border-slate-800 print-card bg-slate-950/30">
            ...
            <div className="my-3.5">
              ...
              {classAverageAttendance !== null && (
                <div className="flex items-center space-x-1.5 mt-1.5 bg-cyan-950/20 border border-cyan-500/10 rounded-lg px-2 py-1 w-fit">
                  <TrendingUp className="h-3 w-3 text-cyan-400" />
                  <span className="text-[9px] font-bold text-cyan-400">
                    {attendancePercentage >= classAverageAttendance 
                      ? `+${attendancePercentage - classAverageAttendance}% above class average (${classAverageAttendance}%)`
                      : `-${classAverageAttendance - attendancePercentage}% below class average (${classAverageAttendance}%)`
                    }
                  </span>
                </div>
              )}
          ...
          <div className="glass-panel p-5 rounded-2xl flex flex-col justify-between border-slate-800 print-card bg-slate-950/30">
            ...
            <div className="my-3.5">
              ...
              {classAverageSubmissionRate !== null && (
                <div className="flex items-center space-x-1.5 mt-1.5 bg-purple-950/20 border border-purple-500/10 rounded-lg px-2 py-1 w-fit">
                  <TrendingUp className="h-3 w-3 text-purple-400" />
                  <span className="text-[9px] font-bold text-purple-400">
                    {Math.round((submittedCount / (totalAssignments || 1)) * 100) >= classAverageSubmissionRate
                      ? `+${Math.round((submittedCount / (totalAssignments || 1)) * 100) - classAverageSubmissionRate}% above class average (${classAverageSubmissionRate}%)`
                      : `-${classAverageSubmissionRate - Math.round((submittedCount / (totalAssignments || 1)) * 100)}% below class average (${classAverageSubmissionRate}%)`
                    }
                  </span>
                </div>
              )}
            ...
            <div className="flex justify-between items-center text-[10px] text-slate-500 border-t border-slate-900 pt-2 font-mono">
              ...
              <span className={closedCount > 0 ? 'text-rose-500 font-semibold' : ''}>{closedCount} closed</span>
              <span className={lateCount > 0 ? 'text-amber-500' : ''}>{lateCount} late</span>
            </div>
          </div>
          ...
          <div className="glass-panel p-5 rounded-2xl flex flex-col justify-between border-slate-800 print-card bg-slate-950/30">
            ...
            <div className="my-3.5">
              ...
              <div className={`flex items-center space-x-1.5 mt-2 px-2.5 py-1 rounded-lg border w-fit ${overallFeedbackStatus.badgeBg}`}>
                <Sparkles className={`h-3 w-3 ${overallFeedbackStatus.iconColor}`} />
                <span className="text-[10px] font-extrabold tracking-wide">
                  {overallFeedbackStatus.text}
                </span>
              </div>
            </div>
            <div className="flex justify-between items-center text-[10px] text-slate-500 border-t border-slate-900 pt-2 font-mono">
                  ...
                  {attendanceHistory.map(row => (
                    <tr key={row.id} className="hover:bg-slate-950/20">
                      ...
                      <td className="py-2.5 text-right text-slate-500 font-mono text-[10px] print-hide">
                        {row.method === 'qr' ? 'QR Code' : row.method === 'manual' ? 'Manual' : row.status === 'Excused Absence' ? 'Excused' : 'Absent'}
                      </td>
                    </tr>
                  ))}
                </tbody>
