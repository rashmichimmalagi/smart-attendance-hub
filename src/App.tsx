...
import { isSupabaseConfigured, authService, supabase, sandboxDb } from './supabase';
...
export default function App() {
  ...
  const checkUserSession = useCallback(async (isSilent = false) => {
    if (isLoggingOutRef.current) {
      console.log('[Auth Debug] checkUserSession ignored because logout is in progress');
      return;
    }
    try {
      const userRes = await authService.getCurrentUser();
      if (isLoggingOutRef.current) {
        console.log('[Auth Debug] checkUserSession result ignored because logout is in progress');
        return;
      }
      if (userRes.profile && userRes.role) {
        ...
        const redirectPath = localStorage.getItem('redirect_to_after_login');
        if (redirectPath) {
  ...
  useEffect(() => {
    if (localStorage.getItem('logout_reason_inactivity') === 'true') {
      localStorage.removeItem('logout_reason_inactivity');
  ...
  // Auth state change listener for real-time auth event handling
  useEffect(() => {
    if (isSupabaseConfigured && supabase) {
      const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
        console.log(`[Auth Debug] onAuthStateChange event received: ${event}`, { userId: session?.user?.id });
        if (event === 'SIGNED_OUT' || !session || isLoggingOutRef.current) {
          console.log('SIGNED_OUT received');
  ...
  const executeLogout = async () => {
    ...
    if (isLoggingOutRef.current) {
      console.log('[Auth Debug] Logout already in progress, ignoring duplicate execution');
      return;
    }
    ...
    try {
      ...
      // Clear all authentication state & cached user information
      setProfile(null);
      setRole(null);
      sandboxDb.currentUser = null;

      // Clear cached session / local storage items
      localStorage.removeItem('redirect_to_after_login');
      try {
      ...
      for (let i = localStorage.length - 1; i >= 0; i--) {
        ...
        if (key && (key.startsWith('sb-') || key.includes('auth') || key.includes('supabase') || key.includes('student') || key.includes('admin'))) {
          // Do NOT remove local database tables (starting with attendance_hub_) or archive metadata
  ...
  const handleLogout = async (skipConfirm?: boolean) => {
    console.log('Sign Out button clicked');
    if (skipConfirm === true) {
      await executeLogout();
    } else {
  ...
  console.log('ProtectedRoute decision', { isAuthenticated: !!profile, role: role || null });

  return (
    <div className="min-h-screen bg-slate-950 relative overflow-hidden">
      <BackgroundBubbles />
      {profile && <InactivityTimeout onLogout={handleLogout} />}
      ...
      <div className="relative z-10">
        {!profile || !role ? (
            ...
            <div className="glass-panel max-w-md w-full p-6 md:p-8 rounded-3xl relative flex flex-col items-center bg-slate-950 border border-slate-900 text-center space-y-6 shadow-2xl animate-fade-in animate-duration-300">
              ...
              <div className="border-t border-slate-900/60 w-full pt-4">
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-300 hover:text-white transition-all font-semibold text-sm cursor-pointer flex items-center justify-center space-x-2 shadow-lg font-sans disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoggingOut ? (
                    <>
            ...
            <div className="glass-panel max-w-md w-full p-6 md:p-8 rounded-3xl relative flex flex-col items-center bg-slate-950 border border-slate-900 text-center space-y-6 shadow-2xl animate-fade-in animate-duration-300">
              ...
              <div className="border-t border-slate-900/60 w-full pt-4">
                <button
                  ...
                  disabled={isLoggingOut}
                  onClick={handleLogout}
                  className="w-full py-3 px-4 rounded-xl bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-300 hover:text-white transition-all font-semibold text-sm cursor-pointer flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoggingOut ? (
                    <>
                      <RefreshCw className="h-4 w-4 animate-spin text-rose-400" />
          ...
          <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center font-sans relative overflow-hidden p-4">
            <BackgroundBubbles />
            <div className="glass-panel max-w-md w-full p-6 md:p-8 rounded-3xl relative flex flex-col items-center bg-slate-950 border border-slate-900 text-center space-y-6 shadow-2xl animate-fade-in animate-duration-300">
              <div className="w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-550 flex items-center justify-center shadow-[0_0_25px_rgba(245,158,11,0.2)]">
                <AlertCircle className="h-8 w-8 text-amber-400 animate-bounce" />
              </div>
              
              <div className="space-y-2">
                <h2 id="suspended-header" className="font-display font-extrabold text-white text-xl tracking-tight">
                  Your account has been suspended.
                </h2>
                <p id="suspended-text" className="text-slate-400 text-sm leading-relaxed">
                  Your student access has been temporarily restricted by an administrator. Please resolve this with faculty.
                </p>
              </div>
