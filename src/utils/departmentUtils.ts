...
export const getDeptAbbreviation = (dept: string | null | undefined): string => {
  if (!dept) return "CSE";
  const upper = dept.toUpperCase();
  if (upper.includes("COMPUTER")) return "CSE";
  if (upper.includes("INFORMATION")) return "ISE";
  ...
  if (upper.includes("INTELLIGENCE") || upper.includes("AIML") || upper.includes("AI & ML") || upper.includes("AI/ML")) return "AIML";
  if (upper.includes("BIOTECH")) return "BT";
  if (upper.includes("AUTOMOBILE")) return "AU";
  if (upper.includes("AERO")) return "AE";
  
  if (upper.includes("MBA") || upper.includes("BUSINESS")) return "MBA";
  
  // Get initials of words
  const initials = dept
    .split(/[\s&/_-]+/)
    .map(w => w[0])
    .join('')
    .toUpperCase();
  return initials.slice(0, 4) || "ADM";
};
