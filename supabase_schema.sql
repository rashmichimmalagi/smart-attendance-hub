...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    account_status TEXT DEFAULT 'Pending' CHECK (account_status IN ('Pending', 'Approved', 'Suspended', 'Rejected')) NOT NULL,
    authentication_code TEXT UNIQUE,
...
CREATE TABLE IF NOT EXISTS public.sessions (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    session_owner_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
    authorized_admin_ids UUID[] DEFAULT '{}'::uuid[],
...
-- Ensure absolute database-level uniqueness across session name, date, and start time to prevent duplicate sessions
ALTER TABLE public.sessions DROP CONSTRAINT IF EXISTS uq_sessions_name_date_time;
ALTER TABLE public.sessions ADD CONSTRAINT uq_sessions_name_date_time UNIQUE (name, date, start_time);
...
CREATE TABLE IF NOT EXISTS public.attendance (
    ...
    scanning_admin_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
    scanning_admin_name TEXT,
...
CREATE TABLE IF NOT EXISTS public.assignments (
    ...
    created_by_user_id UUID, -- Explicit field matching Requirement 1
    created_by_name TEXT, -- Explicit field matching Requirement 1
...
CREATE POLICY "Admins can insert sessions as owner" 
...
-- Admins can update/delete attendance manually if they are owner or authorized
...
USING (
    EXISTS (
        ...
        AND (s.session_owner_id = auth.uid() OR auth.uid() = ANY(s.authorized_admin_ids) OR s.session_owner_id IS NULL)
    )
)
WITH CHECK (
    EXISTS (
...
-- Policies for absence request attachments
CREATE POLICY "Students can upload absence attachments" 
ON storage.objects FOR INSERT TO authenticated 
WITH CHECK (bucket_id = 'absence-attachments' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Anyone authenticated can view absence attachments" 
ON storage.objects FOR SELECT TO authenticated 
USING (bucket_id = 'absence-attachments');

CREATE POLICY "Students can delete their own uploaded absence attachments" 
ON storage.objects FOR DELETE TO authenticated 
USING (bucket_id = 'absence-attachments' AND auth.uid()::text = (storage.foldername(name))[1]);
...
CREATE TABLE IF NOT EXISTS public.absence_requests (
    ...
    history_timeline JSONB DEFAULT '[]'::jsonb NOT NULL,
    CONSTRAINT uq_absence_requests_student_session UNIQUE (student_id, session_id)
);
...
-- Students can delete (withdraw) their own pending requests
